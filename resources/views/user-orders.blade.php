            ...
            <div class="container mx-auto px-6">
                ...
                <div x-show="activeTab === 'current'">
                    <div class="grid md:grid-cols-2 gap-8">
                        @php
                            $currentOrders = $orders->whereIn('status', ['accepted', 'pending', 'completed'])->filter(fn($item) => $item->reviews->isEmpty());
                        @endphp
                ...
                <div x-show="activeTab === 'past'" style="display: none;">
                    <div class="grid md:grid-cols-2 gap-8">
                        @php
                            $pastOrders = $orders->where('status', 'completed')->filter(fn($item) => $item->reviews->isNotEmpty())->merge($orders->where('status', 'declined'));
                        @endphp
